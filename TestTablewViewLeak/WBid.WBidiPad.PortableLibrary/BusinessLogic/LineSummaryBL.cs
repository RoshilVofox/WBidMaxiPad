...
//			}
			...
			//Read all columns from Column definition.xml
			GlobalSettings.AdditionalvacationColumns=  GlobalSettings.columndefinition.Where(p=>p.DisplayName!="TopLock"&&p.DisplayName!="BotLock").OrderBy(z=>z.Order).Select(x => new AdditionalColumns() { DisplayName = x.DisplayName, Id = x.Id, IsRequied = x.IsRequied,DataPropertyName=x.DataPropertyName }).ToList();
			...
			var additionalcolumns= GlobalSettings.AdditionalvacationColumns.Where(x => x.IsRequied == false).OrderBy(x => x.DisplayName);
			GlobalSettings.AdditionalvacationColumns = GlobalSettings.AdditionalvacationColumns.Where(x => x.IsRequied).ToList();
			GlobalSettings.AdditionalvacationColumns.AddRange(additionalcolumns);
