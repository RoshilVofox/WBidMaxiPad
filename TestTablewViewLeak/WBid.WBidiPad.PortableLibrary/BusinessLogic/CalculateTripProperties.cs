                ...
                if (!record.Value.ReserveTrip)
                {
                    ...
                    foreach (var dp in record.Value.DutyPeriods)
                    {
                        ...
                        foreach (var flt in dp.Flights)
                        {
                            ...
                            var ihawaicities = GlobalSettings.WBidINIContent.Cities.Where(x => x.Hawai).Select(y => y.Name);
                            if (ihawaicities.Contains(flt.ArrSta) || ihawaicities.Contains(flt.DepSta))
                            {
                                flt.ETOPS = true;
                            }
                        }
        ...
        private void UpdatePropertyValuesforFlightAttend(Dictionary<string, Trip> tripdata, List<CityPair> ListCityPair)
        {
            ...
            foreach (var record in tripdata)
            {
                ...
                if (!record.Value.ReserveTrip)
                {
                    ...
                    record.Value.Tafb = tripLastFltLandTime - tripFirstFltDepTime + 90;
                    record.Value.RigTafb = record.Value.Tfp < (record.Value.Tafb / 60 * GlobalSettings.FaTafbFactor) ? record.Value.Tafb / 60 * GlobalSettings.FaTafbFactor - record.Value.Tfp : 0m;
