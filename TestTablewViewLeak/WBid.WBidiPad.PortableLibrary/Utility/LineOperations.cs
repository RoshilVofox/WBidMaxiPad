...
namespace WBid.WBidiPad.PortableLibrary.Utility
{
    public static class LineOperations
    {
        ...
        {
			SelectedRows = SelectedRows.OrderBy(y => GlobalSettings.Lines.Select(x => x.LineNum).ToList().IndexOf(y)).ToList();
			foreach (var line in GlobalSettings.Lines)
			{
				line.ManualScroll = 0;
			}
			foreach (var linenum in SelectedRows)
			{
				//showing blue line border for the moved lines
				GlobalSettings.Lines.FirstOrDefault(x => x.LineNum == linenum).ManualScroll=2;
			}
			bool isNeedtoShowBlueLineonFirstLine = false;
			//show blue line for the line just above last selected line
			var lastlineindex= GlobalSettings.Lines.IndexOf(GlobalSettings.Lines.FirstOrDefault(x => x.LineNum == SelectedRows[SelectedRows.Count - 1]));
			//if (lastlineindex > 0)
			if (!SelectedRows.Contains(GlobalSettings.Lines[0].LineNum))
			{
				//GlobalSettings.Lines[lastlineindex - 1].ManualScroll = 1;
				var availablelines = GlobalSettings.Lines.ToList();
				availablelines.RemoveAll(x => x.ManualScroll == 2);
				var bluelineborder = availablelines[lastlineindex - SelectedRows.Count].LineNum;
