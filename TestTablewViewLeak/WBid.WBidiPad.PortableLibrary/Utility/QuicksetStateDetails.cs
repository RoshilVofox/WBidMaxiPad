            ...
            if (appliedStateType.Key == "Block Sort")
            {
				...
				foreach(var items in QuickSetCSW.SortDetails.BlokSort)
				{
					if (items != string.Empty)
						blockSortitems.Add(WBidCollection.GetBlockSortListData().First(y => y.Id.ToString() == items).Name);
				}
...
//                });
        ...
        private void ConstraintsStates()
        {
            ...
            if (states.ReportRelease == null)
                states.ReportRelease = new StateStatus { Cx = false, Wt = false };
			...
			if (states.Commute == null)
				states.Commute = new StateStatus { Cx = false, Wt = false };

			if (states.Commute.Cx)
			{
				CommuttabilityConstraints ();
			}
            //if (states.SDO.Cx)
            //{
        ...
        private void DaysOfWeekConstraint()
        {
            ...
            foreach (Cx3Parameter Cx3Parameter in cxDaysOfWeek.lstParameters)
            {
				...
				var display = getWeekDaysString(Cx3Parameter.ThirdcellValue) + "  " + GetConstraintType(Cx3Parameter.Type) + "  " + Cx3Parameter.Value;
                appliedStateType.Value.Add(display);
            }
            appliedStateType.Key = "Days of Week";
            ConstraintsType.Add(appliedStateType);
        }
		private string getWeekDaysString(string weekday)
		{
			string result = string.Empty;
			switch (weekday)
			{
		...
		private void CitiesLegsConstrain()
		{
			appliedStateType = new AppliedStateType();

			Cx3Parameters cxcity = QuickSetCSW.Constraints.CitiesLegs;
			appliedStateType.Value = new List<string>();
			foreach (Cx3Parameter Cx3Parameter in cxcity.lstParameters)
			{
        ...
        private void StartDayOfTheWeekConstraint()
        {
			...
			Cx3Parameters cxRest = QuickSetCSW.Constraints.StartDayOftheWeek;
            appliedStateType.Value = new List<string>();
            foreach (Cx3Parameter Cx3Parameter in cxRest.lstParameters)
            {
				var display = getWeekDaysString(Cx3Parameter.ThirdcellValue) + "  " + GetConstraintType(Cx3Parameter.Type) + "  " + Cx3Parameter.Value.ToString();
                appliedStateType.Value.Add(display);
            }
		...
		private void CommuttabilityConstraints()
		{
			appliedStateType = new AppliedStateType();
			Commutability commute = QuickSetCSW.Constraints.Commute;
			appliedStateType.Value = new List<string>();
			...
			if (commute != null)
			{
				...
				string secondvalue=string.Empty;
				if (commute.SecondcellValue == 1)
					secondvalue = "NoMiddle";
				else if (commute.SecondcellValue == 2)
					secondvalue = "OKMiddle";
				
				string thirdvalue=string.Empty;
				if (commute.ThirdcellValue == 1)
					thirdvalue = "Front";
				else if (commute.ThirdcellValue == 2)
					thirdvalue = "Back";
				else if (commute.ThirdcellValue == 3)
					thirdvalue = "Overall";
				
				string type=string.Empty;
				if (commute.Type == (int)ConstraintType.LessThan)
					type = "Less Than or equal";
				else if (commute.Type == (int)ConstraintType.MoreThan)
					type = "More Than or equal";
				

				appliedStateType.Value.Add(commute.City+" "+ secondvalue + "  " + thirdvalue );
				appliedStateType.Value.Add(type+" "+ commute.Value.ToString()+"%");

			}
			appliedStateType.Key = "Commuttability";
			ConstraintsType.Add(appliedStateType);
        ...
        private void AppliedWeights(QuickSetCSW quickSetCsw)
        {
			...
			//--------------------------------------------------------------------------------------
			if (cxWtState.CLAuto.Wt)
			{
				appliedStates.AppliedStateTypes.Add(CommutableLineAutoWeight(quickSetCsw.Weights.CLAuto));
			}
			if (cxWtState.Commute == null)
				cxWtState.Commute = new StateStatus { Cx = false, Wt = false };
			if (cxWtState.Commute.Wt) 
			{
				appliedStates.AppliedStateTypes.Add(CommuttabilityWeights(quickSetCsw.Weights.Commute));
			}
            //Days of week   
            //--------------------------------------------------------------------------------------
            if (cxWtState.DOW.Wt)
		...
		private AppliedStateType CommuttabilityWeights(Commutability commute)
		{
			var appliedStateType = new AppliedStateType {Key = "Commtability", Value = new List<string>()};
			...
			if (commute != null)
			{
				...
				//var display =commute.City+" "+ secondvalue + "  " + thirdvalue + "  " +type+"\n "+ commute.Value.ToString()+" "+commute.Weight;
				appliedStateType.Value.Add(commute.City+" "+ secondvalue + "  " + thirdvalue + "  " );
				appliedStateType.Value.Add(type +" "+commute.Value.ToString()+"% : Weight = "+commute.Weight);
        ...
        {
            var appliedStateType = new AppliedStateType {Key = "Commutable Lines - Manual", Value = new List<string>()};
		...
		private AppliedStateType CommutableLineAutoWeight(WtCommutableLineAuto wtCommutableLineAuto)
		{
			var appliedStateType = new AppliedStateType { Key = "Commutable Line-Auto", Value = new List<string>() };

			if(wtCommutableLineAuto!=null)
			{
				appliedStateType.Value.Add(((wtCommutableLineAuto.NoNights) ? "No Nights in Middle" : "Nights in Middle") + Splitter + ((wtCommutableLineAuto.ToWork) ? "To Work "+Splitter : "")  + ((wtCommutableLineAuto.ToHome) ? "To Home"+Splitter : "")  + wtCommutableLineAuto.Weight);
			}
			return appliedStateType;
		}
